...
# Using the `twoStageDesignTMLE` package

## Defining key data objects and possible candidate learners
First, we set up some data objects for the `twoStageDesignTMLE` package. The main object is a `data.frame` of the confounder(s) that are prone to missingness (here, just $W$) _with no missing data_. Importantly, in the syntax of the `tmle` package, `"A"` refers to the exposure (our $X$) and `"W"` refers to the complete set of confounders (our $(W,Z)$).
...
library("xgboost") 
library("ranger") 
library("twoStageDesignTMLE")

# gradient boosted trees, from package xgboost 
# set up a list of tuning parameters with varying maximum tree depth and 
# shrinkage rate
xgb_tune_params <- list(max_depth = c(1, 3), shrinkage = c(1e-2, 1e-1), 
                        ntrees = 500)
...
rf_tune_params <- list(num.trees = 500, min.node.size = min_node_sizes,
                       verbose = FALSE)
# create all possible boosted tree and RF learners based on the tuning 
...
# for two-stage TMLE, need to either specify "Y.orig" on RHS of piform below
# or create a SL wrapper that also uses "Y.orig" (in this case, for SL.glm)
SL.glm.twostage <- function(Y, X, newX, family, obsWeights, model = TRUE, 
                            ...) {
  # check to see if Y is part of X or newX; change name to "Y.orig"
  contains_Y <- (names(X) == "Y")
  if (any(contains_Y)) {
    names(X)[contains_Y] <- "Y.orig"
  }
  
  # fit SL.glm with the new Y names
  SL.glm(Y = Y, X = X, newX = newX, family = family, obsWeights = obsWeights, 
         model = model, ...)
}
```
...
miss_lib <- c("SL.glm.twostage", xgb_learners$names, rf_learners$names)

# obtain marginal estimates using TMLE
set.seed(20240227)
marginal_tmlem_ests <- twoStageTMLE(
  Y = data$Y, A = data$X, 
  # here, W refers to the completely-observed confounders, our "Z"
  W = data[, "Z", drop = FALSE], # need W to be a data.frame
  # here, W.stage2 refers to the complete-case sample of missing-prone 
  # confounders (our W)
  W.stage2 = obs_W, Delta.W = data$Delta,
...
set.seed(20240227)
conditional_tmlem_ests <- twoStageTMLEmsm(
  Y = data$Y, A = data$X, 
  # here, W refers to the completely-observed confounders, our "Z"
  W = data[, "Z", drop = FALSE], # need W to be a data.frame
  # here, W.stage2 refers to the complete-case sample of missing-prone confounders
  # (our W)
  W.stage2 = obs_W, 
  # V refers to the variables to condition on in the MSM
  V = data[, c("X", "Z", "W_obs")], Delta.W = data$Delta,
  MSM = msm_formula, condSetNames = c("W", "A", "Y"),
  pi.SL.library = miss_lib, V.pi = 10,
  Q.family = "binomial",
  Q.SL.library = outcome_regression_lib, V.Q = 10, Qform = outcome_regression_formula,
  g.SL.library = propensity_lib, V.g = 10, gform = propensity_formula,
  augmentW = FALSE, verbose = FALSE
)
...
# obtain marginal estimates using TMLE
set.seed(20240227)
marginal_tmlemto_ests <- twoStageTMLE(
  Y = data$Y, A = data$X, 
  # here, W refers to the completely-observed confounders, our "Z"
  W = data[, "Z", drop = FALSE], # need W to be a data.frame
  # here, W.stage2 refers to the complete-case sample of missing-prone 
...
msm_formula <- "A + Z + W_obs"
set.seed(20240227)
conditional_tmlemto_ests <- twoStageTMLEmsm(
  Y = data$Y, A = data$X, 
  # here, W refers to the completely-observed confounders, our "Z"
  W = data[, "Z", drop = FALSE], # need W to be a data.frame
  # here, W.stage2 refers to the complete-case sample of missing-prone confounders
